...
namespace {
struct MapboxVectorTileTileset {
  std::string url;
  uint32_t level;
};
} // namespace
...
class TMSVectorTileProvider final
    : public QuadtreeVectorOverlayTileProvider {
public:
  TMSVectorTileProvider(
      ...
      const std::vector<MapboxVectorTileTileset>& tileSets)
...
private:
  ...
  std::vector<MapboxVectorTileTileset> _tileSets;
};

TileMapServiceVectorOverlay::TileMapServiceVectorOverlay(
...
//c++23 新特性，std::expected<T, E>正常情况下表示T，非正常情况下表示E。同一时刻只能表示一个值, 要么表示正常值, 要么表示非正常的值。
using GetXmlDocumentResult = nonstd::expected<
    std::unique_ptr<tinyxml2::XMLDocument>,
    VectorOverlayLoadFailureDetails>;
              ...
              -> Future<GetXmlDocumentResult>
        {
            ...
            if (pResponse == nullptr)
            {
              return asyncSystem.createResolvedFuture<GetXmlDocumentResult>(
                  nonstd::make_unexpected(VectorOverlayLoadFailureDetails
                    {
                      VectorOverlayLoadType::TileProvider,
                      std::move(pRequest),
                      "No response received from Mapbox Vector Tile."
                    }));
            }

            const gsl::span<const std::byte> data = pResponse->data();

            std::unique_ptr<tinyxml2::XMLDocument> pDoc = std::make_unique<tinyxml2::XMLDocument>(
            ...
            {
              hasError = true;
              errorMessage = "Unable to parse Mapbox Vector Tile XML document.";
            }
            else
            {
              ...
              if (pRoot == nullptr)
              {
                hasError = true;
                errorMessage =
                    "Mapbox Vector Tile XML document does not have a root "
                    "element.";
              }
              else
              {
                ...
                if (srs != nullptr)
                {
                  std::string srsText1 = srs->GetText();
                  if (srsText1.find("4326") == std::string::npos &&
                      srsText1.find("3857") == std::string::npos &&
                      srsText1.find("900913") == std::string::npos)
                  {
                    hasError = true;
                    errorMessage = srsText1 + " is not supported.";
                  }
                }
                else
                {
...
TileMapServiceVectorOverlay::createTileProvider(
    const CesiumAsync::AsyncSystem& asyncSystem,
      ...
      .thenInMainThread(
              ...
              GetXmlDocumentResult&& xml) -> CreateTileProviderResult
        {
            ...
            std::string fileExtension = options.fileExtension.value_or(
                getAttributeString(pTileFormat, "extension").value_or("pbf"));
            ...
            std::vector<MapboxVectorTileTileset> tileSets;
            ...
            if (pTilesets) {
              ...
              while (pTileset) {
                MapboxVectorTileTileset& tileSet = tileSets.emplace_back();
                tileSet.level =
            ...
            return new TMSVectorTileProvider(
                pOwner,
                asyncSystem,
                pAssetAccessor,
                credit,
