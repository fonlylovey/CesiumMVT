...
namespace Cesium3DTilesSelection 
{
    ...
    CesiumGeometry::QuadtreeTileID MVTUtilities::GetWTMSTileID(const TileID& tileId) 
    {
        struct Operation {
            ...
            CesiumGeometry::QuadtreeTileID operator()(const CesiumGeometry::QuadtreeTileID& quadtreeTileId) {
                 double wmtsY = glm::pow(2, quadtreeTileId.level) - 1.f - (double)quadtreeTileId.y;
            ...
            CesiumGeometry::QuadtreeTileID operator()(const CesiumGeometry::OctreeTileID& octreeTileId) {
                double wmtsY = glm::pow(2, octreeTileId.level) - 1.f - (double)octreeTileId.y;
               return CesiumGeometry::QuadtreeTileID(
                    octreeTileId.level,
                    octreeTileId.x,
                    (uint32_t)wmtsY);
            ...
            CesiumGeometry::QuadtreeTileID operator()(const CesiumGeometry::UpsampledQuadtreeNode& upsampledQuadtreeNode) 
            {
               double wmtsY = glm::pow(2, upsampledQuadtreeNode.tileID.level) - 1.f - (double)upsampledQuadtreeNode.tileID.y;
               return CesiumGeometry::QuadtreeTileID(
                   upsampledQuadtreeNode.tileID.level,
                   upsampledQuadtreeNode.tileID.x,
                   (uint32_t)wmtsY);
            }
        };
        return std::visit(Operation{}, tileId);
    ...
    std::vector<uint32_t> MVTUtilities::TriangulateLineString(
