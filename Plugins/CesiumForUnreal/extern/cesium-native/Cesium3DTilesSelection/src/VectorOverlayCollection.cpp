...
void VectorOverlayCollection::add(
    const CesiumUtility::IntrusivePointer<VectorOverlay>& pOverlay) {
  ...
  IntrusivePointer<VectorOverlayTileProvider> pPlaceholder =
  ...
  CesiumAsync::Future<VectorOverlay::CreateTileProviderResult> future =
      pOverlay->createTileProvider(
          this->_externals.asyncSystem,
          this->_externals.pAssetAccessor,
          this->_externals.pCreditSystem,
          this->_externals.pPrepareMapResources,
  ...
  forEachTile(*this->_pLoadedTiles, [&](Tile& tile) {
    ...
    if (tileState != TileLoadState::Unloaded &&
      ...
      tile.getMappedVectorTiles().push_back(VectorMappedTo3DTile(
  ...
  // destroyed. But it does not keep the VectorOverlayCollection itself alive.
  std::move(future)
      .catchInMainThread(
          [](const std::exception& e)
              -> VectorOverlay::CreateTileProviderResult {
      ...
      .thenInMainThread([pOverlay, pList, pLogger = this->_externals.pLogger](
                            VectorOverlay::CreateTileProviderResult&& result) {
        if (result) {
...
void VectorOverlayCollection::remove(
  ...
  auto removeCondition = [pOverlay](
                             const VectorMappedTo3DTile& mapped) noexcept {
    return (
  ...
  auto pRenderWorker =
      this->_externals.pPrepareMapResources.get();
  forEachTile(
      *this->_pLoadedTiles,
      [&removeCondition, pRenderWorker](Tile& tile) {
        std::vector<VectorMappedTo3DTile>& mapped = tile.getMappedVectorTiles();

        for (VectorMappedTo3DTile& rasterTile : mapped) {
          if (removeCondition(rasterTile)) {
            rasterTile.detachFromTile(*pRenderWorker, tile);
          }
        }
  ...
  auto it = std::find_if(
      list.overlays.begin(),
      list.overlays.end(),
      [pOverlay](const IntrusivePointer<VectorOverlay>& pCheck) noexcept {
...
const std::vector<CesiumUtility::IntrusivePointer<VectorOverlay>>&
VectorOverlayCollection::getOverlays() const {
  if (!this->_pOverlays)
 ...
 */
const std::vector<CesiumUtility::IntrusivePointer<VectorOverlayTileProvider>>&
VectorOverlayCollection::getTileProviders() const {
  if (!this->_pOverlays)
...
const std::vector<CesiumUtility::IntrusivePointer<VectorOverlayTileProvider>>&
VectorOverlayCollection::getPlaceholderTileProviders() const {
  if (!this->_pOverlays)
    return emptyTileProviders;
...
VectorOverlayTileProvider* VectorOverlayCollection::findTileProviderForOverlay(
    VectorOverlay& overlay) noexcept {
  // Call the const version
  const VectorOverlayTileProvider* pResult = this->findTileProviderForOverlay(
      const_cast<const VectorOverlay&>(overlay));
  return const_cast<VectorOverlayTileProvider*>(pResult);
}

const VectorOverlayTileProvider*
VectorOverlayCollection::findTileProviderForOverlay(
...
VectorOverlayCollection::findPlaceholderTileProviderForOverlay(
    VectorOverlay& overlay) noexcept {
  // Call the const version
  const VectorOverlayTileProvider* pResult =
...
VectorOverlayCollection::const_iterator
VectorOverlayCollection::begin() const noexcept {
  if (!this->_pOverlays)
...
VectorOverlayCollection::const_iterator
VectorOverlayCollection::end() const noexcept {
  if (!this->_pOverlays)
...
size_t VectorOverlayCollection::size() const noexcept {
  if (!this->_pOverlays)
    return 0;
