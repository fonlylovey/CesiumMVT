 ...
 *
 * 4. Returning {@link CesiumGltf::VectorModel} means that this tile has glTF model
 */
using VectorTileContentKind = std::variant<
    TileUnknownContent,
    TileEmptyContent,
    TileExternalContent,
    CesiumGltf::VectorModel>;

/**
 * @brief Store the result of loading a tile content after
 * invoking {@link TilesetContentLoader::loadTileContent}
 */
struct CESIUM3DTILESSELECTION_API VectorTileLoadResult {
  /**
   * @brief The content type of the tile.
   */
  VectorTileContentKind contentKind;
